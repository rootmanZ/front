  ...
  export default {
    ...
    data: function () {
      return {
        ...
        conditionalMenuId: this.$route.query.conditionalMenuId,
    ...
    methods: {
      ...
      saveAndPublish() {
        ...
        if (this.menuName === '') {
          this.$Message.error('个性化菜单名称不能为空')
          return
        }
        //当选择个性化菜单时校验表达
        if (this.menuType === 1 && !this.checkMatchRule()) {
          return
        }
        this.$Modal.confirm({
          title: '发布确认',
          content: '发布成功后会覆盖原版本，且将在24小时内对所选用户生效，确认发布？',
          onOk: () => {
            this.menus.matchRule = this.matchRule
            save({
              appId: this.appId,
              id: this.menuId,
              name: this.menuName,
              ...
              this.toMenuManage(this.appId, this.appName)
              this.close()
            })
          }
        })
      ...
      checkMatchRule() {
        ...
        if (this.matchRule.tagId === null) {
          this.$Message.error("标签选项不能为空")
          checkResult = fasle
        }
        return checkResult
      },
      ...
      close() {
        this.closeTag({
          ...
          query: {
            ...
            type: this.type,
            conditionalMenuId: this.conditionalMenuId,
